...
15. **Observation and control are separate contracts.** A callable Harness mutation is not automatically a human-safe UI action. Expose it only when the owning seam defines lifecycle, authorization, scheduling, and model-awareness for human control. This rule has changed its answer once and keeps its principle. An earlier generation made `ctx.jobs.kill()` itself claim the job's terminal delivery, so a human cancel would have left the model believing its work still ran, and `/work` observed jobs without cancelling them. The adopted generation separates the two: the registry owns cancellation, and `dsh-tool-jobs` owns a private ledger of the jobs its own `job_kill` already delivered. A human kill therefore enters no ledger, leaves the owning agent's ordinary completion notice due with the reason merged into its detail, and `/work` offers it from a Job's detail stage behind a two-press confirmation using upstream's own `cancelled by the user`. What earns a control is still the owning seam's explicit semantics rather than the mere existence of a method: continuable subagent interrupt carries human authority because `ctx.subagents` models it, and `/work` still offers nothing over a workflow run because `ctx.workflowEngine` publishes `start()` alone.
...
`userQuestions`, `tokenMeter`, `compaction`, `skills`, `authorization`,
`requestHeader` today) by name — see `tools/capability-probes.mjs` and
