...
`/permission <preset>` switch and an independent `sandbox/mode` or
`approval/policy` change both repaint it live, with no restart or reattachment,
and a reopened Session shows the permission restored from its own log. The
segment is one opaque, escaped id with no inferred risk treatment, and it is
omitted entirely when the deployment composes no permission capability.
