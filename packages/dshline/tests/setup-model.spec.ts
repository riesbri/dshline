...
function facts(overrides: Partial<SetupFacts> = {}): SetupFacts {
  return {
    node: '24.4.0',
    dshline: '0.17.0',
    harness: { kind: 'match', version: '0.1.5-rc.1' },
    profile: 'dshline',
    connect: reading(),
    selected: undefined,
    reason: 'no-route',
...
describe('comparing Harness generations', () => {
  it('matches only on exact equality, as the repository target check does', () => {
    expect(compareGenerations('0.1.5-rc.1', '0.1.5-rc.1')).toEqual({ kind: 'match', version: '0.1.5-rc.1' })
    expect(compareGenerations('0.1.5-rc.1', '0.1.2-rc.1'))
      .toEqual({ kind: 'mismatch', adopted: '0.1.5-rc.1', installed: '0.1.2-rc.1' })
  })

  it('claims nothing when either side could not be read, keeping the half it has', () => {
    expect(compareGenerations('0.1.5-rc.1', undefined))
      .toEqual({ kind: 'unknown', adopted: '0.1.5-rc.1', installed: undefined })
...
describe('the setup report', () => {
  ...
  it('ticks a matching Harness generation and names it', () => {
    expect(row(setupChecks(facts()), 'Harness')).toEqual({ mark: '✓', text: '0.1.5-rc.1' })
  })

  it('warns on a mismatch with both versions and both alignment commands', () => {
    const harness = row(
      setupChecks(facts({ harness: { kind: 'mismatch', adopted: '0.1.5-rc.1', installed: '0.1.2-rc.1' } })),
      'Harness',
    )
    expect(harness.mark).toBe('⚠')
    expect(harness.text).toContain('0.1.2-rc.1 installed')
    expect(harness.text).toContain('dshline targets 0.1.5-rc.1')
    ...
    expect(harness.text).toContain('npm install -g @deepseek-ai/dsh@0.1.5-rc.1')
  })
  ...
  it('marks an unreadable generation unknown rather than good or bad', () => {
    const unknown = row(
      setupChecks(facts({ harness: { kind: 'unknown', adopted: '0.1.5-rc.1', installed: undefined } })),
      'Harness',
    )
