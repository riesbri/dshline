...
On native Windows it asks for a second feature as well: *win32-input-mode*, the Windows console's own way of reporting a key's modifiers. The Windows console does not implement the kitty request, so without it `shift-enter` stays indistinguishable there. A console that does not implement the mode ignores the request, and nothing changes.
...
On a terminal that ignores both requests, `shift-enter` still sends the message. That is why the status line suggests `alt-enter` instead: `alt-enter` works everywhere. The extra modes are switched off when the interface exits, so the next program reads your keyboard normally.
