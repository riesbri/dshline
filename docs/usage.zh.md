...
每百万 token 美元。总输入——未缓存、缓存读取与缓存写入合计——超过 272k token 的请求按长上下文档计费，即 2× 输入、1.5× 输出并应用于**整个请求**：gpt-5.4、gpt-5.5 与 gpt-5.6 系列发布了这一档（gpt-5.4-mini 没有）。5.6 系列在每个档上都按未缓存输入费率的 1.25× 收取缓存写入；gpt-5.4、gpt-5.4-mini 与 gpt-5.5 没有发布缓存写入费率，因此写入不花钱。`gpt-5.6-sol` 的价格至少到 2026-11-21 都是促销价。目录中的 `gpt-5.3-codex-spark` 没有公开 API 等价物，因此保持不定价。这里没有任何东西是你的订阅费用或 Codex 积分——dshline 两者都不核算。
