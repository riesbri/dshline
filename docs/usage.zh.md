...
整行按你写的样子作为你的消息发送。Harness 在它自己的边界识别 `/review-pr` 这个引用，并把该技能的指令放进同一步，因此模型在回答之前就已经拥有它们。这里不会改写这一行的任何部分，会话记录显示的就是你的提示。Harness 注入的指令块**不会**作为第二条会话记录消息回显出来：它是发给模型的提示内容，不是你输入的东西，把它打印回来等于在你的历史里放上一条你从未发送过的消息。

**当模型自己选了一个，证据就是那次工具调用。** 你并没有输入那个名字，所以没有哪一行提示可供寻找。模型调用了 Harness 普通的 `skill` 工具，而那次调用会像任何其他工具调用一样画出卡片：

```
◇ Load skill cordis-composition-reference
```

这一行就是那份持久记录：它在会话记录里，重新打开会话后依然在，并且点名了模型实际选择的技能。完整正文随后作为那次调用的结果到达，会话照常继续。

**技能是一次调用，不是一种模式。** 加载完成之后，输入框和状态行里刻意**没有**「已激活技能」的徽标，任何地方也**没有** `skill: <name>` 这样的片段，因为 Harness 并不暴露持久的 active-skill 状态——这样的徽标没有可报告的东西，只会承诺一个并不存在的模式。技能改变的是模型在该会话余下时间里拥有的指令；它不改变的是本界面认为正在运行的是什么。所以流程就是：
...
> [!NOTE]
> 没有任何东西被预加载。模型的目录里带有每个技能的**名称与描述**；**完整的 `SKILL.md` 正文**只在被调用时才取回。描述是否与当前任务匹配，是模型的判断，而不是一个规则引擎：它自行判断某个技能是否明确适用，然后调用 `skill` 工具。它认为不相关的技能不会被加载，无论其描述看起来多么契合。
