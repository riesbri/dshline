...
编辑按键：`←` `→` 移动，`home` 与 `end`（或 `ctrl-a` 与 `ctrl-e`）到行的两端，`backspace` 与 `delete`，以及 `ctrl-u`、`ctrl-k`、`ctrl-w` 分别删除到行首、到行尾、按词删除。光标通常在末尾时，`ctrl-u` 会清空整条草稿；光标在中间时，先按 `ctrl-a` 再按 `ctrl-k` 也可以。在跨行换行的提示中，`↑` 与 `↓` 也穿过折行上下移动，在短行之间保持你瞄准的列。
...
| | |
| --- | --- |
| `/model` | 更改模型。接受一个名字（`/model deepseek-v4-pro`）或打开一个可输入的选择器；在其中按 `ctrl-k` 可为 subagent 授权模型 |
...
| `/session` | 检视附着在此终端的这场对话，并打开它可用的操作 |
| `/worktrees` | 选择你的 Harness 会话历史所代表的工作目录，然后选择那里的一场对话或开一场新的 |
| `/todos` | 打开当前 Harness Todo 列表的有界只读视图 |
| `/turns` | 打开本会话轮次的有界索引；可检视其中一轮，并按轮次编号或预览文本过滤。`/turns <文本>` 打开时即预先过滤 |
...
在一个已经能发送回合的会话里手动运行 `/setup` 是一次**健康检查，而不是菜单**。它会打印同样的报告，然后以 `✓ Ready.` 返回；它不会打开动作选择器，因为 `/model` 与 `/connect` 才是更改一个可用安装的命令，而 setup 的职责是让你到达可用状态，而不是提供可选的修改。
...
```
⚠ Harness    0.1.2-rc.1 installed · dshline targets 0.1.6-alpha.2
  dshline supports one Harness generation at a time.
  Install the generation this dshline targets: npm install -g @deepseek-ai/dsh@0.1.6-alpha.2
...
其中只有第一条是确定性的，措辞也这样说。这个构建所面向的版本是报告本来就持有的事实；而是否存在某个*已发布的* dshline 面向你已安装的那个版本，则不是——要确定它就意味着把各个发布版本对照它们的 peer 钉住值去解析。因此第二个方向是作为一个条件给出的，而不是作为一个修复。**已确认的不一致会阻止启动。**dshline 打印这份报告，然后拒绝打开会话——它只支持自己所采纳的世代，因此为另一个世代构建的 Host 不保证提供这个构建所面向的 API，而进入输入框只会把失败挪到更难读懂的地方。报告会一直留在屏幕上，直到你安装目标世代并重新启动；`ctrl-d` 退出。Harness 不发布运行时版本服务，因此任一侧读不出来的版本会标记为 `·`，并且不在任何方向上作出断言——读不出版本是一种诊断，而不是不兼容。
...
**Models** 打开模型目录。对于带显式 `models` 列表的路由，列表里的条目就是你要写入的内容。对于继承适配器目录的路由，已存储的自定义会以 override 的形式显示，旁边是已安装的模型；编辑其中一个时，只会写入你实际改动的那些精确路径 `modelOverrides.<model>.<field>`，目录服务的其他每个模型、以及该模型你没有打开过的每个字段都不受影响。一条路由绝不会同时带有显式列表和 override，因为 `llm-pi-ai` 会拒绝这种组合；**Reset models to adapter catalog** 会丢弃该列表。

在一个模型内部，**Display name**、**Context window** 与 **Max output tokens** 写入每个提供方都理解的字段。**Advanced** 则承载本界面从适配器自己的 schema 读取、而非从写死在 dshline 里的列表取得的两项能力：

- **Input modalities** 是端点接受的请求类型——`text`、`image`，或未来适配器新增的任何类型。只声明 text 可以修正一个网关实际服务范围小于目录记录的路由模型；声明 image 才能让手工声明的视觉模型真正可用。空选择会先继承已安装目录的答案，然后是路由默认值。
- **Reasoning capability** 要么复用已安装目录的能力，要么对不推理的模型显式设为 **disabled**，要么为每个提供的级别自定义 dispatch 应发送的 wire 取值。当适配器自己的 schema 声明某个级别的取值可以为空时，该级别也可以映射为不发送任何值——究竟哪个级别可以，是适配器的规则，而不是本界面的。映射由 Harness 在写入处校验：被它拒绝的映射会让表单保持你构建时的原样，并在屏幕上给出它自己的解释。

每个子菜单都只是草稿。在你选择 **Save changes** 之前什么都不写，而那是一次带修订检查的写入；**Discard changes** 与 `esc` 完全不写。如果编辑器打开期间命名空间发生了变动，草稿会被保留、编辑器会说明这一点，第二次明确的 **Save** 只会把你草稿中改动过的那些精确字段重新应用到当前修订号——绝不自动重试，也绝不会写回你没有打开过的字段的陈旧副本。在编辑器下方被移除的路由不会被重新创建：草稿会保留以供查看，直到你退出。

仍然属于设置工作的部分：`compat`、重试策略、超时、图片预算与传输方式留在 `settings.yaml` 里。编辑 `/connect` 显示的内容，永远不会扰动它没有渲染的字段。见 [通过网关访问 DeepSeek](#reaching-deepseek-through-a-gateway)。
...
一行就是一个标题加一个相对年龄，并带有两个简短的关系提示。`open` 标记本窗口已经在驱动的会话——也就是重新打开会拒绝的那一行；`delegated` 标记子会话，这样它不会被误认成无关的对话。元数据先出现；仍在读取精确标题的行会显示 `loading title…`。可能过期的 Harness 投影提示会明显加上 `~`，只有精确观测确认没有标题时才会显示 `untitled`（当前打开的会话则显示 `current`）。如果所有标题结算前就输入标题查询，浏览器会说明还可能出现更多匹配，而不会声称没有匹配。

关于一个会话的其他一切都只差一次按键：`→` 显示它的工作区、创建时间、上次活动时间、日志有多少事件、Harness 是把它作为存活的还是持久化的持有、它的 fork 或委派父级，以及它的 id。在已经 hydration 的行之间移动不会读取事件正文；新进入视口的行可能请求精确标题，而离开浏览器会放弃这些工作。
...
| | |
| --- | --- |
| `Find in this session` | 通过 `searchEvents` 搜索*某一个*会话说过的内容，有自己的查询行（`tab` 执行搜索）；对命中按 `↵` 通过 `readEvent` 打开有界的前后上下文 |
...
在 `Find in this session` 中，对匹配事件按 `↵` 会打开一个有界上下文检视器：通过 `readEvent()` 取得该事件本身，以及它前后固定数量的原始 Harness 事件。目标会被标记，相邻事件保持原有顺序，每个事件的文本来自 Harness 自己的语义提取，因此结构性事件只贡献它的类型与序号。检视器以终端为界，用 `↑`/`↓` 滚动，按 `esc` 或 `ctrl-c` 关闭并回到原样的搜索——查询、结果、选择与滚动位置都不受影响，且在打开某个命中之前不会读取任何上下文。
...
即使重新打开仍然失败——不可读的日志、不兼容的格式版本、没有持久化后端——窗口打印原因并重新打开浏览器，让你选别的。在那里用 `esc` 关闭浏览器会结束这次打开：窗口退出，并且不会开始任何你没要求的会话。

### Session

`/session` 是已附着到这个窗口的那场对话的终端本地 hub。四个相关视图回答的是不同的问题：
...
它的摘要来自所附 agent 的 `Session` 与不可变 header：标题、工作目录、id，以及可用的投影统计。它不会为了绘制 hub 而先去会话语料库查找这场对话。已挂载能力支持时，动作列表打开已有的呈现：

| 操作 | 打开 |
| --- | --- |
| `Find in conversation` | 单个会话搜索及其有界的命中上下文 |
| `Turns` | 本会话的轮次大纲 |
| `Lineage` | 已知的父级与子级对话图 |
| `Rename` | 这个已附着会话的标题编辑器 |

不可用的操作会被省略：Find 与 Lineage 需要相应的会话查询能力，Turns 需要轮次大纲投影，Rename 需要 `ctx.sessionTitle`。Find 直到你提交查询才会搜索，并且只有选中命中后才读取周围事件上下文。Lineage 直到你激活该操作才查询。
...
**一个会话一个进程。**Harness 目前不为持久会话发布跨进程所有权，因此 `/worktrees` 与 `/sessions` 都无法判断某个会话是否已经在别处打开。不要同时在两个 dshline 进程里重新打开同一个会话：随包发布的 JSONL 持久化要求每个会话只有一个存活写入者，而跨进程没有任何东西强制这一点。同时在好几个目录里工作是没问题的，那正是多个终端的用途——没有定义行为的是同一个**会话**出现在两个地方。
...
工作流、subagent 与任务保持独立分区，因为它们是彼此独立的 Harness 权威，而 dshline 不猜测两条能力记录描述同一个操作。它确实展示的那一条关系是发布出来的，而不是猜的：工作流成员携带它启动的 subagent 的 `childId`，因此那个子级出现在它的工作流之下，而不是在扁平的 Subagents 分区里第二次出现。工作流运行在这里完全没有控制手段，因为 `ctx.workflowEngine` 只把运行句柄交给启动它的调用方。状态行的 `work` 片段统计的正是这个浮层所显示的内容，因此呈现在工作流之下的子级不会在那里再被算作一个游离的 subagent。

`/work` 只显示**活动**工作，别无其他。一条任务在 `running` 或 `stopping` 时出现；而它一旦结算——完成、失败或被杀——它的行就消失了，若当时正打开着它的详情视图，那个视图也随之消失。这是产品选择而非能力限制：任务的输出环是一个实时预览，而 `/work` 是一个实时界面，因此已结算的任务不会被留下来供人再次查看。**`/work` 不是任务历史。**已完成工作的持久记录是会话记录本身，恢复会话时会重放它。

一条运行中的任务可以在它的详情阶段用两次 `k` 停止：第一次为那一条任务上膛，页脚显示 `press k again to stop`，第二次则请求 Harness 以 `cancelled by the user` 为由取消它。上膛绑定的是那条任务自身的标识，而不是某个行号，因此导航到另一条任务、关闭该视图、或该任务消失，都会解除上膛；它也会在几秒后过期。处于 `stopping` 的任务不提供停止动作，因为已经没有什么可请求的了。这与网页 UI 上的人类停止按钮调用的是同一个注册表取消操作，而拥有该任务的 agent 仍会收到通常的完成通知——dshline 既不抑制它，也不代表你对模型说话。若生产者拒绝取消，失败会在浮层中短暂显示，该任务继续运行。
...
那是生产者自己的文本，按原样显示：dshline 不解析它、不把它变成百分比，也不编造它未被给出的分母。没有发布进度行的生产者就没有进度，此时显示时钟。终端变窄时丢弃的是整个事实，而不是把某个事实截断：`127/2` 与 `127/203` 是不同且更小的说法，所以进度行比时钟先被丢弃，留下的是任务的种类与标签。

在运行中的任务上按 `↵` 会读取它保留下来的输出。这是与模型所用读取相互独立的一次读取：dshline 用自己的字节偏移向注册表索取输出，这不会移动模型的游标，因此在这里查看任务输出不可能从即将告知 agent 的内容里拿走任何一个字节。该视图显示所保留内容中最新的部分，并且是有界的——很长的任务不会让 dshline 的内存增长。只有在某条任务的视图真正打开时才会读取输出，因此浏览列表、移动高亮或离开该视图都不花费任何代价。

输出行是任务的实时预览，而不是一个终端：它们与其它一切一样绘制在这个有界视图内部，绝不会被提交到它背后的滚动缓冲区，其中包含的任何控制字符都会被显示而不是被执行。`stdout`、`stderr` 与 `log` 靠颜色而非每行前缀来区分，顺序则是注册表记录下来的顺序。

在更早的输出不可获得之处，会有一行如实说明：

```
… earlier output not retained …
```

当注册表自身的保留策略丢掉了流的开头、当生产者报告了一处缺口、或当 dshline 自己的显示上限淘汰了更旧的行时，就会出现这一行。每段缺失只出现一次，其余部分保持可读——一个悄悄把自己拼接起来的预览，会比一个承认存在缺口的预览更糟。
...
`/turns` 打开本会话轮次的有界索引。每一行是一轮——Harness 分配的轮次编号，以及 Harness 从持久日志折叠出的提示或响应预览。记录本身从不被重写：原生滚动缓冲区始终是记录，而大纲是 Harness `turnOutline` 投影之上的视图，不是对话的第二份副本。

- `↑` / `↓` 移动选择；`home` / `end` 跳到第一轮或最后一轮。
- `enter` 打开所选轮次权威提示与响应预览的只读视图。
- `/` 开始过滤。匹配是对轮次编号、提示预览与响应预览做不区分大小写的子串匹配；选择跟随轮次而非行，因此实时更新或过滤变化都不会把你的操作挪到另一轮上。`ctrl-u` 清空过滤。
- 在检视视图内，`←` / `→` 走到上一轮或下一轮，`↑` / `↓` 滚动过长的预览。
- `esc` 关闭，或从检视视图回到大纲。

检视是只读的。一轮的提示或响应预览可能为空——一轮可以在没有合格提示的情况下开始，已完成的轮次也可以没有助手文本——因此界面会说明该预览未被记录，而不是猜测这一轮是否仍在运行。轮次身份、`turn/start` 边界、预览以及已完成轮次的折叠都由 Harness 拥有，因此没有挂载轮次大纲的配置会如实说明；不存在回退折叠。
...
`ctrl-o` 打开被隐藏的行。在新近完成的工具卡片被截断时，它在那个卡片上打开一个检视器——相同的呈现、可滚动、预算远大于卡片本身——并在 `esc` 关闭，你的滚动缓冲区保持原样。这在 `compact` 或 `full` 下都有效。没有这样的卡片等待时，`ctrl-o` 改为循环切换每个*未来*卡片显示多少：`compact`、`full`、`hidden`。已打印的卡片绝不会重绘，这是保持正常终端选择与复制的代价。状态栏的空间会留给会话控制，而不再宣传这个检视快捷键。

在检视器内部，`←` 移到较旧的保留卡片，`→` 移到较新的卡片；`↑`/`↓` 滚动当前卡片，`home`/`end` 跳到其顶部或底部，`esc` 关闭。`ctrl-o` 在这里仍可用作移到较旧卡片的快捷键。标题会数出你所在的位置（`Tool output 2/6`），导航在两端停下，而不是绕回。最近十二张被截断的卡片就这样保持可达，因此你滚动越过的某个结果，不会被其后的工具调用夺走。每张卡片只提供一次：在最新的那张未看过的卡片之后，`ctrl-o` 回到细节循环，这正是让那个开关始终只差一次按键的原因。
...
| `goal armed` | 已设置目标并将自行继续。尚未取任何一轮 |
| `goal 3/256` | 已取三轮，上限 256 |
| `goal idle` | 已设置目标，但本次会话不会继续它。`/goal resume` 武装它 |
...
两种模式都不会在终端变窄时被放弃。它们只在模型名、总计、条形与上下文读数都消失之后才被丢弃，而运行中的目标是最后离开的——在按键提示之后。模式整块丢弃而非缩短：`goal 12/25` 不是比 `goal 12/256` 更小的真相，它是不同的一个。页脚只携带这种紧凑的状态与进度；`/goal` 仍然是查看目标文本的界面。

与这些常驻模式不同，刚刚应用的变更可以在几秒钟内接管状态行的活动片段：上下文压缩、已落地的模型或推理变更、以及权限预设切换。每一种都有自己的权威记录——压缩与权限变更是 Harness 支持的持久事件，而本地的模型与推理变更保留其已提交的回滚缓冲区确认——这段短暂的强调只是让该变更不会在忙碌的 agent 持续输出时滚过去。它会自行消失，而更新的变更会取代它。
...
被委派的 subagent 通常继承驱动模型当前使用的路由。Harness 也可以让驱动模型为通过 subagent 工具启动的子级选择另一条路由，且只对该路径允许它选择你已授权的路由。对于 dshline 当前针对的 Harness 版本，该允许列表不覆盖在 `workflow` 工具内做出的显式路由选择：工作流脚本只有在明确指定子级路由时，才会到达同一套子级路由而不经过这份列表。该授权是 Harness 设置，而不是每个会话各自的决定：
...
`ctrl-k` 打开 **Subagent models** 编辑器。在你按 `s` 之前不会写入任何内容；`esc` 丢弃草稿。编辑器显示：

- `Selection on/off`——新组合的顶层会话是否从 subagent 工具获得可选模型的委派能力。
- `Allowed`——subagent 工具的委派可以为子级请求的确切 `provider/model` 路由。Selection 开启时显示 `N authorized models`；关闭时显示 `N saved models · inactive`，若从未保存过则显示 `no saved models`。`space`（或 `enter`）切换高亮路由，`/` 开始搜索并随输入过滤列表。
- 已保存但实时目录当前未通告的路由旁标有 `unavailable`。它仍然是已授权且可移除的：目录成员资格只是参考，设置本身才是授权。

`e` 切换 Selection 的开关，关闭时保留这些路由以供日后使用；关闭期间编辑器把保留的列表标为 inactive。`ctrl-r` 重新读取实时目录而不打扰草稿。保存会把两个字段作为一次带修订围栏的变更写入；如果该设置先一步在其他地方被改过，编辑器会报告冲突、保留你的草稿，并且不会自行重试——再按一次 `s` 写入它，或按 `esc` 丢弃。

它是一份用于通过 subagent 工具做出的显式选择的允许列表，并不会把 subagent 强制到那些模型上：一次未指定路由的委派仍然继承父级的，无论那条路由是否在列表里。对于 dshline 当前针对的 Harness 版本，它不覆盖在 `workflow` 工具内做出的显式路由选择。它不是成本路由器，也绝不会替你选择 DeepSeek。变更适用于新会话；当前会话保留它组合时记录的策略，已经在运行或已恢复的会话不会因为这个编辑器而改变，子会话则精确继承其父级记录的策略。
...
`$` 的含义取决于路由。在按量付费路由上它估算你花了多少；在 OpenCode Go 上——你按订阅付费——它是按订阅额度计数的、以美元计量的用量，而不是单独一张账单。在已登录路由上——目前是 `openai-codex`——根本没有按 token 的账单，这个数字是**API 等价成本（API-equivalent cost）**：这些精确的 token 按提供商公开 API 费率计费本应花费多少。`/usage` 会直呼其名，而不是让它读起来像你的账单。
...
**已登录的 Codex 按 OpenAI 公开 API 定价，而不是按你的订阅定价。**`openai-codex` 路由是 ChatGPT 登录，其目录服务的模型 id 也在公开 API 上出售，因此每个已定价的 Codex 模型都携带 API 价格作为其等价物——在 `/usage` 中显示为 `API-equivalent cost` 而不是 `cost`，后者会读成该路由从未产生过的账单。费率为 `developers.openai.com` 的标准（短上下文）档，随本发布构建：
...
每百万 token 美元。总输入——未缓存、缓存读取与缓存写入合计——超过 272k token 的请求按长上下文档计费，即 2× 输入、1.5× 输出并应用于**整个请求**：gpt-5.4、gpt-5.5 与 gpt-5.6 系列发布了这一档（gpt-5.4-mini 没有）。5.6 系列在每个档上都按未缓存输入费率的 1.25× 收取缓存写入；对于 gpt-5.4、gpt-5.4-mini 与 gpt-5.5，官方表没有列出缓存写入费率，随附定价把写入表示为零。`gpt-5.6-sol` 的价格至少到 2026-11-21 都是促销价。目录中的 `gpt-5.3-codex-spark` 没有公开 API 等价物，因此保持不定价。这里没有任何东西是你的订阅费用或 Codex 积分——dshline 两者都不核算。

切换路由的会话会把两种钱分开：`/usage` 按基础各显示一行带标签的金额——`cost` 表示按路由自身计费运行的部分，`API-equivalent cost` 表示在已登录路由上运行的部分——绝不会把两者混在一个被称为其中任一种的总数里。
...
**除非你要求，否则没有任何东西仅按模型 id 定价。**同一个模型经网关由网关按其自身条款计费，因此随附费率钉在 `deepseek-official` 路由上——同样地，也钉在 `openai-codex` 路由上以对应已登录的 Codex：服务 `gpt-5.5` 的网关不会被命名 `openai-codex/gpt-5.5` 的条目定价。没有条目的路由上的模型被计数但不定价——你得到 token 而没有 `$`，这是诚实的读数——而缺失部分会话的总计被标记 `~`，以免被误认为完整账单。

**修正随附费率会保留路由的标签。**配置条目替换该键的随附数字（可选的 `tier` 块可以像 `peak` 替换窗口那样替换长上下文档费率），但替换数字不会改变路由是什么：修正 `openai-codex/gpt-5.5` 仍然读作 `API-equivalent cost`，而 dshline 未随附的路由读作 `cost`——为路由写费率就是声明该路由收取什么。
...
状态行会持续显示这个实际权限，位于模型旁边、会话总计之前。它是 Harness 原始的
`currentValue`——部署定义的预设 id、`auto`，或推导出的 `custom`——由沙箱与审批
旋钮以及预设共同折叠而成，因此其中任何一个发生变化都会重绘它，无需重新打开
任何东西。该片段以不透明 id 呈现，不推断风险配色；当部署没有组合任何权限能力时，
该片段完全省略。
