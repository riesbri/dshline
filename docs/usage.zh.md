...
被委派的 subagent 通常继承驱动模型当前使用的路由。Harness 也可以让驱动模型为子级选择另一条路由，但只允许它选择你已授权的路由。该授权是 Harness 设置，而不是每个会话各自的决定：
...
- `Selection on/off`——新组合的顶层会话是否获得可选模型的委派能力。
- `Allowed N models`——驱动模型可以为子级请求的确切 `provider/model` 路由。`space`（或 `enter`）切换高亮路由，`/` 开始搜索并随输入过滤列表。
- 已保存但实时目录当前未通告的路由旁标有 `unavailable`。它仍然是已授权且可移除的：目录成员资格只是参考，设置本身才是授权。

`e` 切换 Selection 的开关，关闭时保留这些路由以供日后使用。`ctrl-r` 重新读取实时目录而不打扰草稿。保存会把两个字段作为一次带修订围栏的变更写入；如果该设置先一步在其他地方被改过，编辑器会报告冲突、保留你的草稿，并且不会自行重试——再按一次 `s` 写入它，或按 `esc` 丢弃。

它是一份用于显式委派选择的允许列表，并不会把 subagent 强制到那些模型上：一次未指定路由的委派仍然继承父级的，无论那条路由是否在列表里。它不是成本路由器，也绝不会替你选择 DeepSeek。变更适用于新会话；当前会话保留它组合时记录的策略，已经在运行或已恢复的会话不会因为这个编辑器而改变，子会话则精确继承其父级记录的策略。
