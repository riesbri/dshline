...
每百万 token 美元。OpenAI 的长上下文档（输入超过 272k token）未被建模，因此长请求按这些标准费率定价，而 `gpt-5.6-sol` 的价格至少到 2026-11-21 都是促销价。目录中的 `gpt-5.3-codex-spark` 没有公开 API 等价物，因此保持不定价。这里没有任何东西是你的订阅费用或 Codex 积分——dshline 两者都不核算。
...
**除非你要求，否则没有任何东西仅按模型 id 定价。**同一个模型经网关由网关按其自身条款计费，因此随附费率钉在 `deepseek-official` 路由上——同样地，也钉在 `openai-codex` 路由上以对应已登录的 Codex：服务 `gpt-5.5` 的网关不会被命名 `openai-codex/gpt-5.5` 的条目定价。没有条目的路由上的模型被计数但不定价——你得到 token 而没有 `$`，这是诚实的读数——而缺失部分会话的总计被标记 `~`，以免被误认为完整账单。配置条目始终读作 `cost`：为路由写费率就是声明该路由收取什么，因此替换 `openai-codex/gpt-5.5` 会把它重新标记为已计费而不是 API 等价。
