...
**Models** 打开模型目录。对于带显式 `models` 列表的路由，列表里的条目就是你要写入的内容。对于继承适配器目录的路由，已存储的自定义会以 override 的形式显示，旁边是已安装的模型；打开其中一个并编辑，只会写入一条 `modelOverrides.<model>` 条目，目录服务的其他每个模型都不受影响。一条路由绝不会同时带有显式列表和 override，因为 `llm-pi-ai` 会拒绝这种组合；**Reset models to adapter catalog** 会丢弃该列表。
...
- **Reasoning capability** 要么复用已安装目录的能力，要么对不推理的模型显式设为 **disabled**，要么为每个提供的级别（`off`、`low`、`high`……）自定义 dispatch 应发送的 wire 取值。只有 `off` 可以不发送任何值。映射由 Harness 在写入处校验：被它拒绝的映射会让表单保持你构建时的原样，并在屏幕上给出它自己的解释。

每个子菜单都只是草稿。在你选择 **Save changes** 之前什么都不写，而那是一次带修订检查的写入；**Discard changes** 与 `esc` 完全不写。如果编辑器打开期间命名空间发生了变动，草稿会被保留、编辑器会说明这一点，第二次明确的 **Save** 才会把它应用到当前修订号——绝不自动重试。在编辑器下方被移除的路由不会被重新创建：草稿会保留以供查看，直到你退出。
