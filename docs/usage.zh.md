...
其中只有第一条是确定性的，措辞也这样说。这个构建所面向的版本是报告本来就持有的事实；而是否存在某个*已发布的* dshline 面向你已安装的那个版本，则不是——要确定它就意味着把各个发布版本对照它们的 peer 钉住值去解析。因此第二个方向是作为一个条件给出的，而不是作为一个修复。**已确认的不一致会阻止启动。**dshline 打印这份报告，然后拒绝打开会话——为另一个世代构建的 Host 缺少这个构建所调用的 API，而进入输入框只会把失败挪到更难读懂的地方。报告会一直留在屏幕上，直到你安装目标世代并重新启动；`ctrl-d` 退出。Harness 不发布运行时版本服务，因此任一侧读不出来的版本会标记为 `·`，并且不在任何方向上作出断言——读不出版本是一种诊断，而不是不兼容。
