...
被委派的 subagent 通常继承驱动模型当前使用的路由。Harness 也可以让驱动模型为通过 subagent 工具启动的子级选择另一条路由，且只对该路径允许它选择你已授权的路由。在 dshline 当前针对的 Harness 版本中，工作流脚本自己 `agent()` 的路由选择在该列表之外，会到达同一套子级路由但不经过它。该授权是 Harness 设置，而不是每个会话各自的决定：
...
它是一份用于通过 subagent 工具做出的显式选择的允许列表，并不会把 subagent 强制到那些模型上：一次未指定路由的委派仍然继承父级的，无论那条路由是否在列表里。它不覆盖 `workflow` 工具，后者的子级路由由脚本选择。它不是成本路由器，也绝不会替你选择 DeepSeek。变更适用于新会话；当前会话保留它组合时记录的策略，已经在运行或已恢复的会话不会因为这个编辑器而改变，子会话则精确继承其父级记录的策略。
