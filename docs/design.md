...
A consequential change can also borrow the activity segment for a few seconds. Context compaction, an applied model or reasoning change, and a permission-preset switch each print a durable row, but a busy agent can scroll that row away before anyone reads it. So the change briefly takes the place of `activity`, in the warning role, and then whatever the session is doing returns on its own. It is emphasis, never a second record: it is not persisted, it is not replayed from history, and the transcript row it points at is unchanged. There is one slot, and a newer notice replaces the older one rather than queueing behind it.
