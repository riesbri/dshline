...
As the terminal gets narrower, whole hints are dropped rather than cut in half — a hint reading `ctrl-d qui` looks like a bug, not like help. The model name is dropped before the context numbers are: which route a step uses is useful configuration context, but the context reading governs whether the session can still continue, so the reading outranks it under width pressure.
...
The ranges are generated from the Unicode Character Database 17.0.0 by `tools/generate-width-tables.mjs`, not hand-maintained, because a table that trails the release measures a code point the terminal draws two columns as one and shifts every row after it. General_Category is not a terminal `wcwidth` function, so the zero-width table is nonspacing and enclosing marks plus an explicit allowlist of format controls; every other format character is measured one cell, the over-counting direction, and projected from a width-critical label. The invariant those labels hold to is one-directional: the terminal must not draw more cells than the renderer counts. A label drawn inside the live region's frame border therefore projects the code points whose width is in doubt — an East Asian Ambiguous character with no stable canonical decomposition, an emoji or pictograph, a format character outside the allowlist, and the multi-code-point sequences no per-code-point rule can see — while keeping ASCII, wide and zero-width code points, and every unambiguously narrow script. The label is composed with NFC first, so a decomposed Hangul syllable is drawn as its single wide syllable, and a precomposed Latin accent survives as its canonical decomposition: `café` is drawn as a base plus a combining mark rather than replaced by a placeholder.
...
- **Deleting it removes the whole token.** A folded paste is drawn as one thing, so backspace, `delete`, `ctrl-w`, `ctrl-u`, and `ctrl-k` remove it entirely rather than one character. The deletion RANGE is widened to cover any still-collapsed span it touches before the buffer is touched at all, so the label and the text always agree. Touching is not intersecting: two spans that meet at one offset each keep their own token, so backspace at the boundary takes the one on the left and `delete` takes the one on the right.
- **Editing it otherwise unfolds it.** A completion that would replace inside a folded span invalidates the label first, because that is an edit of chosen text rather than a deletion. The same is true once a reader has moved into the span: after that the text is ordinary, and backspace is one character again. Nothing remembers that it used to be a paste.
- **Deleting it wholesale takes the label with it.** Nothing is expanded on the way out.

The numbers are monotonic for the lifetime of one `Composer`. The root session composer keeps its sequence across submissions and clears, so the second large block in a conversation is `#2`; other multiline `Composer` instances, such as a subagent-message editor, maintain their own sequence. A new `Composer` starts again at `#1`. The numbers are never written anywhere durable, and `ctrl-z` does not hand one back — an undone paste does not release its identity, because a reader may still remember `#1` meaning the first block, and giving that number to different content would be a worse lie than a gap in the sequence.
...
The browser's 2.0 capabilities stay on the same authority line. Corpus filters, lineage navigation, within-session search, and cursor-backed paging are all projections of `ctx.sessionQuery` — `filterSessions` clauses for workspace and age, `traceSession` for ancestry, `searchEvents` for one session's hits, opaque cursors for both paged scopes — and rename is the single mutation, delegated to `ctx.sessionTitle` for the session this window drives, whose log-only `session/title` event carries the explicit `user` source. The browser opens small child overlays (filter picker, lineage, event search, rename prompt) from one keystroke each; each child owns its own selection, query, and viewport, and closing it restores the parent's exact state because the two never share live-region state. Which keystroke follows the scope of the question: `ctrl-f` for the filters, which narrow the corpus, and `→` for the surface that discloses one session — its facts, and the actions that only make sense for it. The list keeps only cheap identity cues — title, age, `open`, and `delegated` for child sessions — while the bounded log read behind an event count and a last-activity time happens when that surface is opened, so moving the cursor does not read event bodies; it may request exact titles for newly visible rows.

Title resolution follows the same action boundary. The metadata listing is usable before title hydration; pending and provisional labels are visibly different from exact title and exact absence. Visible rows are hydrated first, a local title query makes unresolved retained titles relevant, and closing or replacing the listing aborts obsolete work. A future Harness `listSessionHints()` observation can supply zero-I/O projection-backed title hints without changing this UI contract; exact title reads remain available for reconciliation.

The within-session search keeps that on-demand shape one level deeper: `searchEvents()` discovers hits, and `readEvent()` is paid only when a hit is explicitly opened, returning the exact target event plus a bounded raw-log window around it. Rendering the hit list or moving through it costs no log read, the catalog aborts a superseded read rather than letting it repaint the wrong row, and every displayed event fact is Harness's — the type, sequence, and time, and the body from Harness's own `extractSessionEventText()` rather than a frontend parser, so a structural or unknown event shows only its type and sequence.
