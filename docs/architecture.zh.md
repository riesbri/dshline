...
Sessions 是指向另一方向的案例。`AgentHandle` 交给创建该 agent 的调用者，其文档说明处置器是那个所有者持有的能力——因此使 agent 退役在这里是被授权的，而重新打开会话是前端可以采取的人类操作。Harness **不**定义的是，当其所属 agent 中途消失时，任务或委派 subagent 应该发生什么，因此窗口在任一者附着时拒绝重新打开，并拒绝在一轮进行中时重新打开，点名原因而不是猜测。浏览器拥有文本输入模式后，会话重命名走上了同一条路：`ctx.sessionTitle` 建模明确的 `user` 权威，因此重命名只对本窗口驱动的会话暴露，绝不作为列出标题的副作用。已关闭的持久会话的重命名仍被推迟——该服务只持活动会话对象。
