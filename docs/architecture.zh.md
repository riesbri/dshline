...
setup 贡献的只是一次读取与一个顺序，此外没有别的。这个顺序由读取中的警告决定：只有当报告带有某个警告时，回答它的修复步骤才会出现，因此一份全清的读取会以 `✓ Ready.` 返回输入框，而不是打开一个提供可选更改的选择器。它由「缺什么」领头：一旦有路由被注册，`Choose a model` 排在第一位；而当 `/connect` 关闭、刚刚产生了第一条可用路由、而选择仍然缺失或陈旧时，指挥者会自己打开选择器，而不是回到一张只会告诉你去打开它的清单。做这件事的是指挥者，永远不是 `/connect`——浏览器仍然只是浏览器，对 setup 一无所知——并且只在模型正是那块缺失的拼图时才做，因为连接第二个提供方并不是更换一个已经可用的选择的请求。
